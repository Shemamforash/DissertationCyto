...
var consoleElements, Evaluator = {
        elements: {
            keywords: ["if", "(", ")", "else", "then", "endif", "INTERNAL", "SOURCE", "SINK", "reset", "min", "max", "VAR"],
            conditional_symbols: ["<", ">", "<=", ">=", "=="],
            operators: ["+", "-", "*", "/", "="],
            input: "",
            button: {}
        ...
        parse_statement: function (tokens) {
            ...
            while (next_token !== undefined) {
                ...
                // else {
                //     var result = Evaluator.parse_conditional(tokens);
                //     tokens = result.tokens;
                //     final_stmt += result.stmt;
                // }
                if (next_token.type === "number") {
                    return next_token.value;
                } else if (next_token.type === "bool") {
                    return next_token.value;
                }
                next_token = tokens.shift();
                return final_stmt;
